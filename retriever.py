...
class FlaxBertEmbeddings(nn.Module):
    ...
    param_dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    ...
    def setup(self):
        ...
        self.LayerNorm = nn.LayerNorm(epsilon=self.config.layer_norm_eps, dtype=self.dtype, param_dtype=self.param_dtype)
...
class FlaxBertAttention(nn.Module):
    ...
    def setup(self):
        self.self = FlaxBertSelfAttention(self.config, causal=self.causal, dtype=self.dtype, param_dtype=self.param_dtype)
        self.output = FlaxBertSelfOutput(self.config, dtype=self.dtype, param_dtype=self.param_dtype)
...
class FlaxBertLayer(nn.Module):
    ...
    def setup(self):
        self.attention = FlaxBertAttention(self.config, causal=self.config.is_decoder, dtype=self.dtype, param_dtype=self.param_dtype)
        self.intermediate = FlaxBertIntermediate(self.config, dtype=self.dtype, param_dtype=self.param_dtype)
        self.output = FlaxBertOutput(self.config, dtype=self.dtype, param_dtype=self.param_dtype)
        if self.config.add_cross_attention:
            self.crossattention = FlaxBertAttention(self.config, causal=False, dtype=self.dtype, param_dtype=self.param_dtype)
...
class FlaxBertLayerCollection(nn.Module):
    ...
    def setup(self):
        if self.gradient_checkpointing:
            ...
            self.layers = [
                FlaxBertCheckpointLayer(self.config, name=str(i), dtype=self.dtype, param_dtype=self.param_dtype)
                for i in range(self.config.num_hidden_layers)
            ]
        else:
            self.layers = [
                FlaxBertLayer(self.config, name=str(i), dtype=self.dtype, param_dtype=self.param_dtype) for i in range(self.config.num_hidden_layers)
            ]
...
class FlaxBertPooler(nn.Module):
    config: BertConfig
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    param_dtype: jnp.dtype = jnp.float32  # the dtype of the paramters
...
class FlaxBertModule(nn.Module):
    ...
    def setup(self):
        self.embeddings = FlaxBertEmbeddings(self.config, 
            dtype=self.dtype, 
            param_dtype=self.param_dtype
        )
        ...
        self.pooler = FlaxBertPooler(self.config, dtype=self.dtype, param_dtype=self.param_dtype)
